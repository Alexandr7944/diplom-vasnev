...
@section('content')
    ...
    <form action="{{ route('cinema.store') }}" method="post">
        @csrf
        <input type="text" name="name" placeholder="Название зала"/>
        <input type="number" name="numberOfRows" placeholder="Количество рядов"/>
        <input type="number" name="numberOfSeat" placeholder="Количество мест"/>
                    ...
                    @foreach($cinemas as $cinema)
                        <li>
                            ...
                            <a href="{{ route('cinema.destroy', $cinema->id) }}"
                               class="conf-step__button conf-step__button-trash">Удалить</a>
                        </li>
                    @endforeach
                </ul>
            </div>
                ...
                <ul class="conf-step__selectors-box">
                    @foreach($cinemas as $cinema)
                        <li><input type="radio" class="conf-step__radio" name="chairs-hall" value="Зал 1" checked><span
                                class="conf-step__selector">{{ $cinema->name }}</span></li>
                        <li>
                    @endforeach
                </ul>
